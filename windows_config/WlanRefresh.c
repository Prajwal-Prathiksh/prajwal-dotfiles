/**
* @file WlanRefresh.c
* @brief Script to force refresh/re-scan wireless networks from the command line.
* 
* This script is part of the Prajwal's dotfiles for Windows configuration.
* It provides functionality to force a refresh or re-scan of wireless networks.
* 
* @see https://superuser.com/questions/889414/force-refresh-re-scan-wireless-networks-from-command-line
* 
*/
...
typedef struct wlan_scan_finished_context
{
    WLAN_INTERFACE_INFO_LIST *interface_list;
    HANDLE semaphore;
} wlan_scan_finished_context;
    ...
    if ((data->NotificationSource | WLAN_NOTIFICATION_SOURCE_ACM) == WLAN_NOTIFICATION_SOURCE_ACM)
    {
        if (data->NotificationCode == wlan_notification_acm_power_setting_change || data->NotificationCode == wlan_notification_acm_scan_complete)
        {
            ...
            if (ctx)
            {
                ...
                {
                    long prev;
                    ReleaseSemaphore(ctx->semaphore, 1, &prev);
                }
            }
        }
    }
...
int _tmain(int argc, TCHAR *argv[])
{
    ...
    else
    {
        ...
        if (wlanapi)
        {
#define X(Module, Return, Name, Params) \
    typedef Return Name##_t Params;     \
    Name##_t *Name = (Name##_t *)GetProcAddress(Module, __CRT_STRINGIZE(Name));
...
#undef X
            ...
            if (result == ERROR_SUCCESS)
            {
                ...
                if (result == ERROR_SUCCESS)
                {
                    ...
                    for (unsigned int i = 0; i != interface_list->dwNumberOfItems; ++i)
                    {
                        ...
                        if (result_i == ERROR_SUCCESS)
                        {
                            ++nwait;
                        }
                        else
                        {
                            interface_list->InterfaceInfo[i].isState = (WLAN_INTERFACE_STATE)(interface_list->InterfaceInfo[i].isState | (result_i << 16));
                        }
                    }
                    while (nwait > 0)
                    {
                    ...
                    if (result == ERROR_SUCCESS && register_notification_result != ERROR_SUCCESS)
                    {
        ...
        {
            result = (unsigned int)GetLastError();
        }
    }
    ...
    printf("Refreshed wireless network list\n");
